...
func TestWithOptionsDynamicConfig(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(200)
		if _, err := w.Write([]byte(`{
				"issuer": "https://example.com",
	...
	config, err := configure(
		WithDiscoveryURL(discoveryURL),
		WithClientID(strPtr("dynamic_client_id")),
		WithClientSecret(strPtr("dynamic_client_secret")),
		WithScopes(&[]string{"dynamic_scope1", "dynamic_scope2"}),
		WithAudience(strPtr("dynamic_audience")),
		WithStorageProvider(&mockStorage),
	)

	assert.NoError(t, err)
	assert.Equal(t, "dynamic_client_id", *config.ClientId)
	assert.Equal(t, "dynamic_client_secret", *config.ClientSecret)
	assert.Equal(t, "https://example.com/device", *config.DeviceAuthorizationEndpoint)
	assert.Equal(t, "https://example.com/token", *config.TokenEndpoint)
	assert.Equal(t, []string{"dynamic_scope1", "dynamic_scope2"}, *config.Scopes)
	assert.Equal(t, "dynamic_audience", *config.Audience)
	assert.NotNil(t, *config.StorageProvider)
}
